...
base class CobradorMCPServer extends MCPServer with ToolsSupport {
  CobradorMCPServer(super.channel)
    : super.fromStreamChannel(
        ...
        instructions:
            'Servidor MCP para gestionar funcionalidades de la aplicación cobrador',
      ) {
    ...
    inputSchema: Schema.object(
      properties: {
        'filter': Schema.string(
          description: 'Filtro opcional para buscar clientes',
        ),
      },
    ),
  );
  ...
  final getPaymentStatusTool = Tool(
    ...
    inputSchema: Schema.object(
      properties: {'client_id': Schema.string(description: 'ID del cliente')},
      required: ['client_id'],
    ),
  );
  ...
  FutureOr<CallToolResult> _createPayment(CallToolRequest request) async {
    ...
    final description =
        request.arguments?['description'] as String? ?? 'Pago registrado';
  ...
  FutureOr<CallToolResult> _getPaymentStatus(CallToolRequest request) async {
    ...
    return CallToolResult(
      content: [
        TextContent(
          text:
              'Estado de pagos para cliente $clientId:\n' +
              '- Pagos pendientes: 2\n' +
